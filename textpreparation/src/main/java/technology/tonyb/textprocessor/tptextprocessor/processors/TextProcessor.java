package technology.tonyb.textprocessor.tptextprocessor.processors;
...
import technology.tonyb.textprocessor.tptextprocessor.stemmer.EnglishStemmer;
import technology.tonyb.textprocessor.tptextprocessor.stemmer.SlovakStemmer;
import technology.tonyb.textprocessor.tptextprocessor.stopwords.CzechSW;
import technology.tonyb.textprocessor.tptextprocessor.stopwords.EnglishSW;
...
public class TextProcessor {
	...
	public static List<Word> removeWordsMadeOnlyFromSpecificChars(List<Word> words) {
		...
		if (words != null && words.size() > 0) {
			...
			for (int i = 0; i < words.size(); i++) {
				if (!words.get(i).getWord().matches("^[.]{1,}$")
						&& !words.get(i).getWord().matches("^[-]{1,}$") 
						&& !words.get(i).getWord().matches("^[–]{1,}$")
						&& !words.get(i).getWord().matches("^[0-9]{1,}$")
						&& !words.get(i).getWord().matches("^[&]{1,}$")) {
		...
		if (words != null && words.size() > 0 && stopWords != null && stopWords.length > 0) {
			...
			for (int i = 0; i < words.size(); i++) {
				...
				for (int j = 0; j < stopWords.length; j++) {
					if (words.get(i) != null && StringUtils.isNotBlank(words.get(i).getWordLowerCase()) && words.get(i).getWordLowerCase().equals(stopWords[j])) {
						isStopWord = true;
						break;
					}
				}
		...
		if (words != null && words.size() > 0) {
			...
			for (int i = 0; i < words.size(); i++) {
				for (int j = i + 1; j < words.size(); j++) {
					if (words.get(i) != null && words.get(j) != null 
							&& StringUtils.isNotBlank(words.get(i).getStemmedWordLowerCaseNoDiacritics())
							&& StringUtils.isNotBlank(words.get(j).getStemmedWordLowerCaseNoDiacritics())
							&& words.get(i).getStemmedWordLowerCaseNoDiacritics().equals(words.get(j).getStemmedWordLowerCaseNoDiacritics())) {
						words.set(j, null);
					}
