...
public class Database {
	...
	public static Word selectRandomWord() {
		...
		try {
			ps = connection.prepareStatement("SELECT w.id, w.uuid, w.number_of_word_occurences_in_text, w.term_frequency, w.stemmed_word_lower_case_no_diacritics"
	...
	public static Word selectWord(int id) {
		...
		if (id >= 0) {
			try {
				ps = connection.prepareStatement("SELECT w.id, w.uuid, w.number_of_word_occurences_in_text, w.term_frequency, w.stemmed_word_lower_case_no_diacritics FROM words AS w WHERE w.id = ?");
				ps.setInt(1, id);
				rs = ps.executeQuery();
	...
	public static TfIdf selectIdfInfo(Word word) {
		...
		if (word != null && StringUtils.isNotBlank(word.getStemmedWordLowerCaseNoDiacritics())) {
			...
			try {
				ps = connection.prepareStatement("SELECT t1.number_of_documents_with_word, t2.number_of_all_documents, t3.number_of_words_in_examined_document FROM "
				...
				if (rs != null && rs.next()) {
					tfidf = new TfIdf()
						.setNumberOfAllDocuments(rs.getInt("number_of_all_documents"))
					...
					logger.debug("Information retrieved for stemmedWordLowerCaseNoDiacritics='" + word.getStemmedWordLowerCaseNoDiacritics() + "' with values: " + tfidf.toString() + " was selected from database.");
				}
