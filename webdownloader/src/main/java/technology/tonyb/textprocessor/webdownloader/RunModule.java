...
import technology.tonyb.textprocessor.tpconfiguration.objects.WebPage;
import technology.tonyb.textprocessor.webdownloader.downloader.JsoupDownloader;
...
public class RunModule {
	...
	public static void main(String[] args) {
		...
		try {
			if (args != null
					&& args[0].equals("-start_with") && StringUtils.isNotBlank(args[1]) 
					&& args[2].equals("-link_must_contains") && StringUtils.isNotBlank(args[3])
					&& args[4].equals("-download_next") && StringUtils.isNotBlank(args[5])
					) {
				...
				logger.error("Invalid number of parameters! Use: -start_with url -link_must_contains base_of_url. E.g.: -start_with https://www.sme.sk -link_must_contains sme.sk -download_next true -cookie_name cookieName -cookie_value cookieValue");
			}
		} catch (Throwable e) {
